...
# ======================================================
    # 📋 СПИСОК ЗАКАЗОВ (CRM 1.2.1 с быстрой навигацией)
    # ======================================================
    if choice == "Список заказов":
        ...
        if resp.data:
            ...
            status_filter = col2.selectbox("Этап проекта", ["Все"] + list(df["status"].unique()))
            responsible_filter = col3.selectbox("Сотрудник", ["Все"] + list(df["Ответственный"].unique()))
            ...
            st.write("---")
            if not df.empty:
                st.write("### ⚡ Быстрое управление")
                nav_col1, nav_col2 = st.columns([3, 1])
                
                # Создаем список для выбора из отфильтрованных данных
                selection_options = {f"Перейти в карточку: {row['client_name']} (ID: {row['id']})": row['id'] for _, row in df.iterrows()}
                selected_for_nav = nav_col1.selectbox("Выберите проект для редактирования:", options=list(selection_options.keys()), label_visibility="collapsed")
                
                if nav_col2.button("🚀 Открыть карточку", use_container_width=True):
                    # Сохраняем ID в session_state и переключаем меню
                    st.session_state.selected_order_id = selection_options[selected_for_nav]
                    # Мы не можем программно изменить selectbox в сайдбаре напрямую, 
                    # но мы сообщим пользователю, что данные подгружены.
                    st.success(f"Заказ ID {st.session_state.selected_order_id} готов к работе в разделе 'Карточка проекта'!")
