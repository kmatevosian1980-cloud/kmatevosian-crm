...
if check_password():
   ...
   # ======================================================
    # 📋 СПИСОК ЗАКАЗОВ (Полная версия)
    # ======================================================
    if choice == "Список заказов":
        st.title("📋 Все текущие проекты")

        # Запрашиваем абсолютно все данные из таблицы orders
        ...
        if resp.data:
            ...
            # РАСШИРЕННЫЙ СПИСОК КОЛОНОК ДЛЯ ТАБЛИЦЫ
            display_df = df[[
            ...
            st.caption(f"Всего заказов: {len(display_df)} | Сумма по всем: {df['total_price'].sum():,.0f} ₽")
        else:
