...
if check_password():
    ...
    if choice == "Список заказов":
        ...
        if resp.data:
            ...
            # Фильтр по статусу
            all_statuses = df_renamed['Статус'].unique().tolist()
            selected_statuses = st.multiselect("Отфильтровать по этапу:", options=all_statuses, default=all_statuses)
            
            filtered_df = df_renamed[df_renamed['Статус'].isin(selected_statuses)]
            
            if selected_cols:
                st.dataframe(filtered_df[selected_cols], use_container_width=True)
                if 'Общая сумма' in selected_cols:
                    st.caption(f"Всего заказов: {len(filtered_df)} | Сумма выбранных: {filtered_df['Общая сумма'].sum():,.0f} р.")
            else:
    ...
    # --- 3. КАРТОЧКА ПРОЕКТА (РЕДАКТОР И ФАЙЛЫ) ---
        ...
        if resp.data:
            ...
            with tab2:
                st.write("### 📎 Документы")
                uploaded_file = st.file_uploader("Загрузить файл", type=['png', 'jpg', 'pdf'])
    ...
    # --- 4. АНАЛИТИКА ---
        ...
        if resp.data:
            ...
            c1.metric("Оборот", f"{df['total_price'].sum():,.0f} р.")
            c2.metric("Касса", f"{df['paid_amount'].sum():,.0f} р.")
            c3.metric("Долги", f"{(df['total_price'] - df['paid_amount']).sum():,.0f} р.")
